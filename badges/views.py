...
from programs.permission_views import LeadMentorRequiredMixin, get_user_role, can_user_write
        ...
        if student_id:
            try:
                ...
                if can_user_read(request.user, "badges", s):
                    student = s
                    earned_ids = set(StudentBadge.objects.filter(student=s).values_list("badge_id", flat=True))
            except: pass
        role = get_user_role(request.user)
        show_how = role in ("LeadMentor","Mentor")
        is_lead = role == "LeadMentor"
        from .models import BadgeCategory
        return render(request, "badges/badge_list.html", {"badges":badges,"earned_ids":earned_ids,"student":student,"show_how":show_how,"awards":StudentBadge.objects.filter(student=student).select_related("badge","awarded_by") if student else [],"is_lead_mentor":is_lead,"categories":BadgeCategory.choices})
...
class BadgeDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["awards"] = StudentBadge.objects.filter(badge=self.object).select_related("student","awarded_by","awarded_by__adult_profile")
        ...
        if program_id:
            try:
                qs = qs.filter(enrollment__program_id=int(program_id), enrollment__active=True).distinct()
            ...
            try:
                ...
                if mm:
                    qs = qs.filter(enrollment__program_id=int(mm.group(1)), enrollment__active=True).distinct()
            except Exception:
                pass
        from django.db.models import Value
        from django.db.models.functions import Coalesce, Lower, NullIf
        ctx["students"] = qs.annotate(
            sort_first=Lower(Coalesce(NullIf("first_name", Value("")), "legal_first_name")),
...
class BadgeCreateView(LeadMentorRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        log_event(event=AuditEvent.BADGE_CREATED, resource=self.object, request=self.request, after={"name": self.object.name, "level": self.object.level, "category": self.object.category}, notes=f"Badge created: {self.object}")
        return resp
...
class BadgeUpdateView(LeadMentorRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        resp = super().form_valid(form)
        log_event(event=AuditEvent.BADGE_UPDATED, resource=self.object, request=self.request, before=before, after={"name": self.object.name, "level": self.object.level}, notes=f"Badge updated: {self.object}")
        return resp
...
class BadgeDeleteView(LeadMentorRequiredMixin, DeleteView):
    ...
    success_url = reverse_lazy("badges:list")
    def form_valid(self, form):
        log_event(event=AuditEvent.BADGE_DELETED, resource=self.get_object(), request=self.request, before={"name": self.get_object().name, "level": self.get_object().level}, notes=f"Badge deleted: {self.get_object()}")
        return super().form_valid(form)

class BadgeAwardView(LoginRequiredMixin, View):
    def post(self, request, pk):
        if get_user_role(request.user) not in ("LeadMentor","Mentor"):
            messages.error(request,"Permission denied"); return redirect("badges:list")
        badge = get_object_or_404(Badge, pk=pk)
        student_id = request.POST.get("student_id")
        student = get_object_or_404(Student, pk=student_id)
        obj, created = StudentBadge.objects.get_or_create(student=student, badge=badge, defaults={"awarded_by":request.user})
        if created:
            log_event(event=AuditEvent.BADGE_AWARDED, resource=obj, request=request, after={"badge": str(badge), "badge_id": badge.pk, "student": str(student), "student_id": student.pk, "awarded_by": str(request.user)}, notes=f"Granted badge '{badge}' (ID {badge.pk}) to student '{student}' (ID {student.pk}) by {request.user}")
...
class BadgeRevokeView(LeadMentorRequiredMixin, View):
    def post(self, request, pk):
        ...
        log_event(event=AuditEvent.BADGE_REVOKED, resource=sb, request=request, before={"badge": str(badge), "badge_id": badge.pk, "student": str(sb.student), "student_id": sb.student.pk}, notes=f"Revoked badge '{badge}' (ID {badge.pk}) from student '{sb.student}' (ID {sb.student.pk}) by {request.user}")
        sb.delete()
