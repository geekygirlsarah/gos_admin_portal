...
def attendance_tap(request):
    ...
    if not program.has_feature("attendance"):
        return JsonResponse(
            {"error": "Attendance is not enabled for this program"}, status=400
        )
...
def student_weekly_hours(request, student_id):
    ...
    sessions = AttendanceSession.objects.filter(
        program=program, student=student, check_in__lt=end
    ).filter(Q(check_out__isnull=True) | Q(check_out__gt=start))
...
def program_weekly_hours(request, program_id):
    ...
    sessions = (
        AttendanceSession.objects.filter(
            program=program, check_in__lt=end, student__isnull=False
        )
        .filter(Q(check_out__isnull=True) | Q(check_out__gt=start))
