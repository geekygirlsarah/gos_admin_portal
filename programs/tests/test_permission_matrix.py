...
from programs.permission_views import can_user_read, can_user_write, get_user_role, user_is_parent
...
class PermissionMatrixTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.mentor_user = User.objects.create_user(username="mentor", password="pass")
        cls.mentor = Adult.objects.create(
            user=cls.mentor_user, legal_first_name="Mentor", last_name="User", is_mentor=True
        )

        cls.parent_user = User.objects.create_user(username="parent", password="pass")
        cls.parent = Adult.objects.create(
            user=cls.parent_user, legal_first_name="Parent", last_name="User", is_parent=True
        )

        cls.student_user = User.objects.create_user(username="student", password="pass")
        cls.student = Student.objects.create(
            user=cls.student_user, preferred_first_name="Student", last_name="User"
        )

        cls.alumni_user = User.objects.create_user(username="alumni", password="pass")
        cls.alumni = Adult.objects.create(
            user=cls.alumni_user, legal_first_name="Alumni", last_name="User", is_alumni=True
        )
        ...
        cls.mentor_default_no_read = {"attendance", "team_assignments", "badge_award", "badge_manage"}
        cls.no_write_except_lead = {"background_checks", "payments", "sliding_scale", "fees"}

    def _set_permission(self, role, section, can_read=True, can_write=False):
        RolePermission.objects.update_or_create(
            role=role, section=section, defaults={"can_read": can_read, "can_write": can_write}
        )
        ...
        for section in self.sections:
            with self.subTest(section=section):
                ...
                if section in self.finance_sections:
                    ...
                    self._set_permission("Mentor", section, can_read=True, can_write=False)
                    self.assertFalse(
        ...
        for section in self.sections:
            with self.subTest(section=section):
                ...
                if section in self.no_write_except_lead:
                    self.assertFalse(
                        can_write,
                        f"Mentor should NOT write {section} (restricted)"
                    )
        ...
        for section in self.sections:
            with self.subTest(section=section):
                ...
                if section in self.no_write_except_lead:
                    self.assertFalse(
                        can_write,
                        f"Parent should NOT write {section} (restricted)"
                    )
                elif section in self.mentor_only_sections:
                    # Mentor-only sections
                    self.assertFalse(can_write, f"Parent cannot write {section} (mentor-only)")
                else:
                    self.assertTrue(
    ...
    def test_student_write_matrix(self):
        ...
        for section in self.sections:
            with self.subTest(section=section):
                ...
                if section in {"outreach", "student_info", "identity", "contact_address", "health_medical", 
                               "school", "cmu_andrew", "student_documents", "discord", "first_website", 
                               "parents_emergency", "other_details", "attendance", "adult_info", "programs"}:
                    ...
                    self.assertTrue(can_write, f"Student should write {section} (can_write=True)")
                else:
                    # Sections like finance, background_checks are restricted
                    if section in self.no_write_except_lead:
                        self.assertFalse(can_write, f"Student cannot write {section} (restricted)")
                    elif section in self.mentor_only_sections:
                        self.assertFalse(can_write, f"Student cannot write {section} (mentor-only)")
    ...
    def test_alumni_write_matrix(self):
        ...
        for section in self.sections:
            with self.subTest(section=section):
                ...
                if section in self.no_write_except_lead:
                    self.assertFalse(can_write, f"Alumni should NOT write {section} (restricted)")
                elif section in self.mentor_only_sections:
                    self.assertFalse(can_write, f"Alumni cannot write {section} (mentor-only)")
                else:
    ...
    def test_parent_can_read_own_student_info(self):
        ...
        self.assertTrue(can_user_read(self.parent_user, "student_info", obj=self.student))
    ...
    def test_parent_cannot_read_other_student_info(self):
        ...
        self._set_permission("Parent", "student_info", can_read=True, can_write=False)
        self.assertFalse(can_user_read(self.parent_user, "student_info", obj=other_student))
    ...
    def test_get_user_role_matrix(self):
        ...
        nobody = User.objects.create_user(username="nobody", password="pass")
...
class PermissionEdgeCaseTests(TestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.lead_mentor = User.objects.create_superuser(username="lead", password="pass")
        lm_group, _ = Group.objects.get_or_create(name="LeadMentor")
        cls.lead_mentor.groups.add(lm_group)
    ...
    def test_background_checks_never_writable_for_non_lead(self):
        ...
        for role_name, user in [
            ...
            with self.subTest(role=role_name):
                RolePermission.objects.update_or_create(
                    role=role_name, section="background_checks",
    ...
    def test_finance_sections_parent_read_only(self):
        ...
        RolePermission.objects.update_or_create(
            role="Parent", section="payments",
            defaults={"can_read": True, "can_write": True}
        )
        RolePermission.objects.update_or_create(
            role="Parent", section="sliding_scale",
            defaults={"can_read": True, "can_write": True}
        )
