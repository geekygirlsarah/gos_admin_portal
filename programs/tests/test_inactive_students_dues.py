...
from programs.models import Enrollment, Program, Student, Fee, FeeAssignment
...
class InactiveStudentsDuesTest(TestCase):
    ...
    def test_inactive_students_separated(self):
        # Active Student
        student_active = Student.objects.create(legal_first_name="Active", last_name="Student")
        Enrollment.objects.create(student=student_active, program=self.program, active=True)
        
        # Inactive Student
        student_inactive = Student.objects.create(legal_first_name="Inactive", last_name="Student")
        Enrollment.objects.create(student=student_inactive, program=self.program, active=False)
        
        # Graduated Student (should also be inactive)
        student_graduated = Student.objects.create(legal_first_name="Graduated", last_name="Student", graduated=True)
        Enrollment.objects.create(student=student_graduated, program=self.program, active=True)
