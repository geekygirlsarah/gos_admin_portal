...
from programs.models import (
    Program, Student, SlidingScale, Fee, Enrollment, RaceEthnicity, StudentApplication, School
)
...
class UtilsAndModelEdgeTests(TestCase):
    def test_compute_sliding_discount_rounding(self):
        ...
        self.assertEqual(compute_sliding_discount_rounded(None, Decimal('10')), Decimal('0'))
        self.assertEqual(compute_sliding_discount_rounded(Decimal('100'), None), Decimal('0'))

    def test_student_eighteenth_birthday_regular_and_leap(self):
        s1 = Student.objects.create(legal_first_name='A', last_name='B', date_of_birth=datetime.date(2010, 5, 20))
        self.assertEqual(s1.eighteenth_birthday(), datetime.date(2028, 5, 20))
        # Feb 29 should map to Feb 28 on non-leap year
        s2 = Student.objects.create(legal_first_name='C', last_name='D', date_of_birth=datetime.date(2008, 2, 29))
    ...
    def test_student_requires_background_check(self):
        ...
        s = Student.objects.create(legal_first_name='A', last_name='B', date_of_birth=datetime.date(2010, 5, 20))
        prog = Program.objects.create(name='Season', start_date=datetime.date(2028, 1, 1), end_date=datetime.date(2028, 12, 31))
        ...
        p3 = Program.objects.create(name='Early', start_date=datetime.date(2028, 1, 1), end_date=datetime.date(2028, 5, 19))
        self.assertFalse(s.requires_background_check(p3))
        # If program is after 18th but start <= end (always) -> True (because end >= birthday)
        p4 = Program.objects.create(name='After', start_date=datetime.date(2028, 5, 21), end_date=datetime.date(2028, 6, 1))
...
class FormBehaviorTests(TestCase):
    def setUp(self):
        ...
        self.enrolled_student = Student.objects.create(legal_first_name='Alex', last_name='Smith')
        self.not_enrolled = Student.objects.create(legal_first_name='Casey', last_name='Doe')
        Enrollment.objects.create(student=self.enrolled_student, program=self.program)
        self.fee_in_prog = Fee.objects.create(program=self.program, name='Dues', amount=Decimal('50.00'))
        self.fee_other = Fee.objects.create(program=self.other_program, name='Kit', amount=Decimal('20.00'))
    ...
    def test_sliding_scale_form_percent_validation_and_queryset(self):
        form = SlidingScaleForm(program=self.program, data={
            'student': self.enrolled_student.pk,
            'percent': '150',  # invalid (>100)
        })
        self.assertFalse(form.is_valid())
        form2 = SlidingScaleForm(program=self.program, data={
            'student': self.enrolled_student.pk,
            'percent': '100',
        })
    ...
    @override_settings(EMAIL_SENDER_ACCOUNTS=[], DEFAULT_FROM_EMAIL='noreply@example.com')
    ...
    @override_settings(EMAIL_SENDER_ACCOUNTS=[
    ...
    def test_program_email_form_sender_accounts(self):
        ...
        self.assertEqual(getattr(form2.fields['program'].widget, 'input_type', ''), 'hidden')
        # If hidden and not provided in data -> non-field error
        form_hidden_missing = ProgramEmailForm(program=p, data={
        ...
        form_hidden_ok = ProgramEmailForm(program=p, data={
            'program': p.pk,
            'recipient_groups': ['students'],
            'subject': 'Hello',
            'body': 'Test',
...
class ApplicationFlowTests(TestCase):
    ...
    def test_student_application_approve_reuses_existing_by_email(self):
        existing = Student.objects.create(legal_first_name='Existing', last_name='Person', personal_email='e@example.com')
        app = StudentApplication.objects.create(
            program=self.program,
        ...
        self.assertTrue(Enrollment.objects.filter(student=existing, program=self.program).exists())
