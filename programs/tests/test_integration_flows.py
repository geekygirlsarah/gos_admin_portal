...
from programs.models import Program, Student, Enrollment, Fee, SlidingScale, Payment, Adult
...
class IntegrationFlowTests(TestCase):
    ...
    def setUp(self):
        ...
        # Setup users
        self.admin = User.objects.create_superuser(username="admin", password="password", email="admin@example.com") # nosec B101
        self.admin.groups.add(self.lead_mentor_group) # Explicitly add to group just in case
        
        self.parent_user = User.objects.create_user(username="parent", password="password", email="parent@example.com") # nosec B101
    ...
    def test_sliding_scale_to_balance_sheet_flow(self):
        ...
        response = self.client.post(fee_url, {
        ...
        self.assertTrue(Fee.objects.filter(program=self.program, name="Registration Fee").exists())

        # 2. Verify student balance sheet currently shows $200
        balance_url = reverse("program_student_balance", args=[self.program.pk, self.student.pk])
