...
def add_andrew_columns_if_missing(apps, schema_editor):
    ...
    cols = [r[1] for r in db.cursor().execute(
        "PRAGMA table_info(programs_adult)"
    ).fetchall()]
    cur = db.cursor()
    ...
    if "andrew_email" not in cols:
        cur.execute("ALTER TABLE programs_adult ADD COLUMN andrew_email varchar(254) NULL")
    if "andrew_id_expiration" not in cols:
        cur.execute("ALTER TABLE programs_adult ADD COLUMN andrew_id_expiration date NULL")
    if "andrew_id_sponsor_id" not in cols:
        cur.execute("ALTER TABLE programs_adult ADD COLUMN andrew_id_sponsor_id bigint NULL REFERENCES programs_adult(id) DEFERRABLE INITIALLY DEFERRED")
