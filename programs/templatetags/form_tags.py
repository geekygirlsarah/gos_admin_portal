...
def add_class(field, css):
    ...
    try:
        return field.as_widget(
            attrs={
                **(field.field.widget.attrs or {}),
                "class": f"{(field.field.widget.attrs.get('class', '') + ' ' + css).strip()}",
            }
        )
    except Exception:
...
def render_field(field, **kwargs):
    ...
    try:
        ...
        if isinstance(widget, widgets.CheckboxInput):
            # Single checkbox
            input_html = field.as_widget(
                attrs={
                    **attrs,
                    "class": f"form-check-input {attrs.get('class', base_classes)}".strip(),
                }
            )
            label_html = (
                field.label_tag(attrs={"class": "form-check-label"})
                if field.label
                else ""
            )
