...
class ApplicationWizardView(View):
    ...
    def get_application(self, request):
        code = request.session.get("application_code") or request.GET.get("code")
        if code:
            return Application.objects.filter(application_code=code).first()
        return None

    def get(self, request, *args, **kwargs):
        app = self.get_application(request)
        if app:
             # Ensure session matches application
             request.session["application_code"] = app.application_code

        if not app:
            # Start new: Role/Program Selection
            form = RoleSelectionForm()
            return render(
                request,
                self.template_name,
                {"form": form, "step": "select_role", "progress": 10},
            )

        # Dispatch based on current_step
        step = app.current_step
        if step == "select_role":
            form = RoleSelectionForm(initial={'role_type': app.role_type, 'program': app.program})
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "select_role", "progress": 10},
            )
        elif step == "identity":
            form = IdentityForm(initial={'email': app.email_to_verify})
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "identity", "progress": 20},
            )
            ...
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "verify_otp", "progress": 30},
            )
        elif step == "verify_parent_otp":
            from .application_forms import VerifyParentOTPForm
            form = VerifyParentOTPForm()
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "verify_parent_otp", "progress": 40},
            )
        elif step == "data_entry":
            if app.role_type == "student_parent":
                # Determine if we need student or parent info based on handoff
                if app.status == "pending_handoff":
                    form = ParentInfoForm(initial=app.data.get("parent_info", {}))
                    step_label = "parent_info"
                else:
                    form = StudentInfoForm(initial=app.data.get("student_info", {}))
                    step_label = "student_info"
            else:
                form = MentorInfoForm(initial=app.data.get("mentor_info", {}))
                step_label = "mentor_info"

            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": step_label, "progress": 50},
            )
        elif step == "disclosures":
            form = DisclosureUploadForm(application=app)
            # Find required disclosure templates
            role_filters = ["all"]
            if app.role_type == "student_parent":
                role_filters.extend(["student", "parent"])
            else:
                role_filters.append("mentor")
            disclosures = DisclosureForm.objects.filter(
                required_for_role__in=role_filters, is_active=True
            )
            return render(
                request,
                self.template_name,
        ...
        return render(request, self.template_name, {"app": app, "step": step})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "back" and app:
            if app.current_step == "identity":
                app.current_step = "select_role"
            elif app.current_step == "verify_otp":
                # Clear OTP if going back from verification
                app.otp_code = None
                app.is_verified = False
                app.current_step = "identity"
            elif app.current_step == "verify_parent_otp":
                app.parent_otp_code = None
                app.is_parent_verified = False
                app.current_step = "data_entry"
                # If we go back from parent OTP, it should be student info step
                app.status = "draft"
            elif app.current_step == "data_entry":
                if app.status == "pending_handoff":
                    # If we were in handoff, going back means going back to parent OTP verification if verified, or student info
                    if app.is_parent_verified:
                        app.is_parent_verified = False
                        app.current_step = "verify_parent_otp"
                    else:
                        app.status = "draft"
                else:
                    app.is_verified = False
                    app.current_step = "verify_otp"
            elif app.current_step == "disclosures":
                app.current_step = "data_entry"
                app.status = "pending_approval" # To allow going back from disclosure if we were there
            app.save()
        ...
        if not app:
            ...
            if form.is_valid():
                ...
                request.session["application_code"] = code
                return redirect("apply_start")
            return render(
                request,
                self.template_name,
        ...
        if step == "select_role":
            form = RoleSelectionForm(request.POST)
            if form.is_valid():
                app.role_type = form.cleaned_data["role_type"]
                app.program = form.cleaned_data["program"]
                app.current_step = "identity"
                app.save()
                return redirect("apply_start")
            return render(
        ...
        if step == "identity":
            ...
            if form.is_valid():
                from .application_forms import generate_otp
                email = form.cleaned_data["email"]
                otp = generate_otp()
                app.email_to_verify = email
                app.otp_code = otp
                app.current_step = "verify_otp"
                app.save()
                # Send OTP via email
                ...
                send_mail(
                    ...
                    f"Your code is {otp}. Use it to verify your email for application {app.application_code}.",
            ...
            if form.is_valid():
                if form.cleaned_data["otp"] == app.otp_code:
                    app.is_verified = True
                    app.current_step = "data_entry"
                    
                    # Pre-fill data if existing records found
                    email = app.email_to_verify
                    if app.role_type == "student_parent":
                        student = Student.objects.filter(personal_email=email).first()
                        if student:
                            app.existing_student = student
                            if "student_info" not in app.data:
                                student_data = {
                                ...
                                # Pre-fill parent email from existing student's primary contact
                                if student.primary_contact:
                                    student_data["parent_email"] = student.primary_contact.email or student.primary_contact.personal_email
                                
                                app.data["student_info"] = student_data
                    else:  # mentor_volunteer
                        adult = Adult.objects.filter(models.Q(email=email) | models.Q(personal_email=email)).first()
                        if adult:
                            app.primary_adult = adult
                            if "mentor_info" not in app.data:
                                app.data["mentor_info"] = {
                                    "legal_first_name": adult.first_name,
                                    "first_name": adult.preferred_first_name,
                                    "last_name": adult.last_name,
                                    "cell_phone_number": adult.cell_phone or adult.phone_number,
                                }
            ...
            if app.role_type == "student_parent":
                if app.status == "pending_handoff":
                    ...
                    if form.is_valid():
                        app.data["parent_info"] = sanitize_for_json(form.cleaned_data)
                    ...
                    if form.is_valid():
                        app.data["student_info"] = sanitize_for_json(form.cleaned_data)
                        # Check for handoff or parent verification
                        parent_email = form.cleaned_data.get("parent_email")
                        app.parent_email_to_verify = parent_email
                        
                        # Generate Parent OTP
                        from .application_forms import generate_otp as gen_otp
                        parent_otp = gen_otp()
                        app.parent_otp_code = parent_otp
                        app.is_parent_verified = False
                        
                        # Determine if older student (handoff)
                        grade_val = form.cleaned_data.get("grade")
                        is_older = grade_val and int(grade_val) >= 7
                        
                        if is_older:
                            app.status = "pending_handoff"
                            email_subject = "Student Application Hand-off"
                            email_body = f"A student has started an application ({app.application_code}). Please complete the parent portion. Your verification code is {parent_otp}. Resume here: {request.build_absolute_uri('/')}apply/?code={app.application_code}"
                        else:
                            app.status = "pending_handoff" # Both flows will now use this to trigger parent verification
                            email_subject = "Your Application Verification Code"
                            email_body = f"Your parent verification code is {parent_otp}. Use it to verify your email for application {app.application_code}."
                        ...
                        if is_older:
                            # Show thank you or info page for older students
                            return render(
                                request, "apply/handoff_sent.html", {"app": app}
                            )
                        else:
                            # Younger student, redirect to parent OTP verification
                            return redirect("apply_start")
                    step_label = "student_info"
            else:
                form = MentorInfoForm(request.POST)
                if form.is_valid():
                    app.data["mentor_info"] = sanitize_for_json(form.cleaned_data)
                    app.status = "pending_approval"
                    app.save()
                    return redirect("apply_thanks")
                step_label = "mentor_info"
            ...
            from .application_forms import VerifyParentOTPForm
            form = VerifyParentOTPForm(request.POST)
            if form.is_valid():
                if form.cleaned_data["otp"] == app.parent_otp_code:
                    app.is_parent_verified = True
                    app.current_step = "data_entry"
                    
                    # Pre-fill parent info if exists
                    parent_email = app.parent_email_to_verify
                    if parent_email:
                        parent = Adult.objects.filter(models.Q(email=parent_email) | models.Q(personal_email=parent_email)).first()
                        if parent and "parent_info" not in app.data:
                            parent_data = {
                                "p1_first_name": parent.first_name,
                                "p1_last_name": parent.last_name,
                                "p1_email": parent.email or parent.personal_email,
                                "p1_phone": parent.cell_phone or parent.phone_number,
                                "p1_relationship": parent.relationship_to_student,
                            }
                            # Also pre-fill secondary parent if existing student
                            if app.existing_student and app.existing_student.secondary_contact:
                                s2 = app.existing_student.secondary_contact
                                parent_data.update({
                                    "p2_first_name": s2.first_name,
                                    "p2_last_name": s2.last_name,
                                    "p2_email": s2.email or s2.personal_email,
                                    "p2_phone": s2.cell_phone or s2.phone_number,
                                    "p2_relationship": s2.relationship_to_student,
                                })
                            app.data["parent_info"] = parent_data
                    
                    app.status = "pending_approval"
                    app.save()
                    return redirect("apply_thanks")
                else:
                    form.add_error("otp", "Invalid Parent OTP code.")
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "verify_parent_otp", "progress": 40},
            ...
            form = DisclosureUploadForm(request.POST, request.FILES, application=app)
            if form.is_valid():
                # Save disclosure uploads
                for field_name, file in request.FILES.items():
                    if field_name.startswith("disclosure_"):
                        df_id = field_name.split("_")[1]
                        df = DisclosureForm.objects.get(pk=df_id)
                        ApplicationDisclosure.objects.create(
                            application=app, disclosure_form=df, signed_pdf_upload=file
                        )
                app.status = "pending_approval"
                app.save()
                return redirect("apply_thanks")
            return render(
                request,
                self.template_name,
                {"form": form, "app": app, "step": "disclosures", "progress": 80},
            )
...
class ReviewApplicationListView(LoginRequiredMixin, LeadMentorRequiredMixin, ListView):
    model = Application
    template_name = "programs/application_review_list.html"
    context_object_name = "applications"

    def get_queryset(self):
        qs = Application.objects.filter(status="pending_approval")
    ...
    def get_context_data(self, **kwargs):
        ...
        program_id = self.request.GET.get("program")
        if program_id:
            ctx["filtered_program"] = Program.objects.filter(id=program_id).first()
        return ctx


class ReviewApplicationDetailView(
    LoginRequiredMixin, LeadMentorRequiredMixin, DetailView
):
    model = Application
    template_name = "programs/application_review_detail.html"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "approve":
            app.status = "pending_disclosures"
            app.save()
            messages.success(request, f"Application {app.application_code} approved.")
            # In a real app, send email here
            return redirect("application_review_list")
        elif action == "deny":
            app.status = "rejected"
            app.save()
            messages.warning(request, f"Application {app.application_code} rejected.")
            return redirect("application_review_list")

        return redirect("application_review_detail", pk=app.pk)


class DisclosureUploadView(View):
    template_name = "apply/wizard.html"

    def get_application(self, request, code):
        return Application.objects.filter(application_code=code).first()

    def get(self, request, code):
        app = self.get_application(request, code)
        if not app:
            messages.error(request, "Application not found.")
            return redirect("apply_start")

        if app.status != "pending_disclosures":
            messages.warning(
                request, "This application is not ready for disclosures yet."
            )
            return redirect("apply_start")

        from .application_forms import DisclosureUploadForm
        form = DisclosureUploadForm(application=app)
    ...
    def post(self, request, code):
        ...
        if form.is_valid():
            form.save()
            # Check if all required disclosures are uploaded
            role_filters = ["all"]
            if app.role_type == "student_parent":
                role_filters.extend(["student", "parent"])
            else:
                role_filters.append("mentor")
            required_count = DisclosureForm.objects.filter(
                required_for_role__in=role_filters, is_active=True
            ).count()
            uploaded_count = app.disclosures.count()

            if uploaded_count >= required_count:
                # In a real app, maybe moves to 'accepted' after mentor final check
                # For now let's say it's done or moves to a 'final_review'
                messages.success(request, "All disclosures uploaded successfully!")
                return redirect("apply_thanks")
            else:
                messages.success(request, "Disclosure uploaded.")
                return redirect("disclosure_upload", code=code)
