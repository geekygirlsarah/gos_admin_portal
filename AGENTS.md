...
- **OutreachEvent / OutreachSignup**: Community events that students can sign up for as "champions" (leads) or "helpers". Outreach events are tied to a specific **Program** and can be enabled or disabled per program via the 'outreach' feature toggle in `ProgramFeature`. Access is program-scoped (e.g. `/programs/<id>/outreach/`).
