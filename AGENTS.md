...
- **TEMPORARY — Mentor Access Block**: `MENTOR_ACCESS_BLOCKED` in `GoSAdminPortal/settings.py` is a temporary flag (removed once mentor portal features are ready) that blocks mentors from the portal. While on: mentor-only users (no Parent/Alumni flags, not a lead mentor) are logged out and bounced to the login page with a message by `TemporaryMentorBlockMiddleware` (`GoSAdminPortal/middleware.py`); mentors who are also parents/alumni stay logged in but `get_user_role` never returns `"Mentor"` and the dashboard's mentor section is hidden via `mentor_access_blocked()` in `programs/permission_views.py`. Lead mentors/superusers are unaffected. The flag defaults to `not TESTING` (mirroring `APPLY_RATE_LIMIT_ENABLED`), so existing mentor tests run with the block off; force it on in tests with `@override_settings(MENTOR_ACCESS_BLOCKED=True)` (see `programs/tests/test_temporary_mentor_block.py`, which should be deleted with the block).
