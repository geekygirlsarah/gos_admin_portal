...
from programs.permission_views import get_user_role, can_user_write, can_user_delete
from programs.views.mixins import DynamicReadPermissionMixin, DynamicWritePermissionMixin
...
class OutreachEventListView(LoginRequiredMixin, OutreachProgramMixin, DynamicReadPermissionMixin, ListView):
    model = OutreachEvent
    ...
    def get_context_data(self, **kwargs):
        ...
        events = self.get_queryset().annotate(
            effective_end_date=Coalesce('end_date', 'start_date')
        ).order_by('start_date', 'start_time')
        
        upcoming_events = events.filter(effective_end_date__gte=today)
        past_events = events.filter(effective_end_date__lt=today).order_by('-start_date', '-start_time')
        ...
        if role == 'Student':
            try:
                ...
                context['my_events'] = upcoming_events.filter(signups__student=student).distinct()
                context['other_events'] = upcoming_events.exclude(signups__student=student)
                context['student_signups'] = OutreachSignup.objects.filter(student=student, event__program=self.program).values_list('event_id', flat=True)
                context['student_signup_roles'] = {s.event_id: s.role for s in OutreachSignup.objects.filter(student=student, event__program=self.program)}
...
class OutreachEventCreateView(LoginRequiredMixin, OutreachProgramMixin, DynamicWritePermissionMixin, CreateView):
    model = OutreachEvent
    ...
    def form_valid(self, form):
        ...
        if role == 'Student':
            try:
                ...
                messages.success(self.request, "Event created and you have been signed up as a champion!")
            except AttributeError:
                messages.warning(self.request, "Event created, but we couldn't find your student profile to sign you up as a champion.")
        else:
            messages.success(self.request, "Event created successfully.")
        return response

class OutreachEventUpdateView(LoginRequiredMixin, OutreachProgramMixin, DynamicWritePermissionMixin, UpdateView):
    model = OutreachEvent
    form_class = OutreachEventForm
    template_name = 'outreach/event_form.html'
    section = 'outreach'
