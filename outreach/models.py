...
class OutreachEvent(models.Model):
    program = models.ForeignKey('programs.Program', on_delete=models.CASCADE, related_name='outreach_events', null=True)
...
class OutreachSignup(models.Model):
    ...
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name='outreach_signups')
    event = models.ForeignKey(OutreachEvent, on_delete=models.CASCADE, related_name='signups')
    ...
    def clean(self):
        if self.role == self.CHAMPION:
            if self.event.champions.exclude(id=self.id).count() >= self.event.max_champions:
                raise ValidationError(f"This event already has the maximum number of champions ({self.event.max_champions}).")
        elif self.role == self.HELPER:
            if self.event.helpers.exclude(id=self.id).count() >= self.event.max_helpers:
                raise ValidationError(f"This event already has the maximum number of helpers ({self.event.max_helpers}).")
