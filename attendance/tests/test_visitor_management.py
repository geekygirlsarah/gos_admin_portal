...
class VisitorManagementTest(TestCase):
    def setUp(self):
        ...
        self.program = Program.objects.create(name="Test Program", start_date=datetime.date(2026, 1, 1))
        # Ensure program has attendance feature
        from programs.models import ProgramFeature
        self.attendance_feature, _ = ProgramFeature.objects.get_or_create(name="Attendance", key="attendance")
    ...
    def test_visitor_name_inconsistency(self):
        ...
        names = list(AttendanceSession.objects.values_list("visitor_name", flat=True).distinct())
    ...
    def test_merge_visitors(self):
        ...
        self.assertEqual(AttendanceSession.objects.filter(visitor_name="John Doe").count(), 2)
        self.assertEqual(AttendanceSession.objects.filter(visitor_name="John").count(), 0)
    ...
    def test_merge_visitors_events_sync(self):
        ...
        self.assertEqual(AttendanceEvent.objects.filter(visitor_name="John Fixed").count(), 1)
