...
class AttendanceHoursChartFilterTest(TestCase):
    def setUp(self):
        ...
        wednesday_dt = datetime(2026, 9, 2, 10, 0, tzinfo=timezone.get_current_timezone())
        AttendanceSession.objects.create(
            program=self.program,
            student=self.student,
    ...
    def test_filter_by_single_day(self):
        ...
        response = self.client.get(self.url, {"program_id": self.program.id, "days_of_week": ["3"]})
    ...
    def test_filter_by_multiple_days(self):
        ...
        response = self.client.get(self.url, {"program_id": self.program.id, "days_of_week": ["2", "4"]})
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_no_matching_days(self):
        ...
        response = self.client.get(self.url, {"program_id": self.program.id, "days_of_week": ["5"]})
        self.assertEqual(response.status_code, 200)
